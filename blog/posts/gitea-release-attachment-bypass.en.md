...
This one also has a subplot worth telling: I initially overscored it, and Medium is the honest number for a reason that involves the exact serving path in `httplib/serve.go`.
...
## Why it is Medium, and how I got there

Originally filed as High, borrowing the parent advisory's `C:H/I:H` vector on the reasoning that a renamed `.html` or `.svg` attachment served from the Gitea origin becomes stored XSS. The maintainer pushed back and said Medium. They were right, and it is worth showing exactly why, because it is a good lesson in not assuming impact you have not verified.

Gitea's file-serving layer is hardened against precisely this. In `modules/httplib/serve.go`, every served file gets a strict Content Security Policy:
...
The `sandbox` token with no `allow-scripts` kills JavaScript execution even when the file is rendered inline. On top of that, user-supplied HTML is forced to `text/plain`:
...
and the default disposition for attachments is `attachment`, with `X-Content-Type-Options: nosniff` set. A renamed `.html` or `.svg` does not execute script on the Gitea origin. The stored XSS I assumed simply does not fire. Once you take XSS off the table, the confidentiality and integrity impact on the CVSS vector drops, and the honest number is Medium.

What the bug actually gives you is the defeat of the operator's allowlist policy: hosting a file under a forbidden extension, for example distributing a `.exe` or `.msi` that masquerades as a legitimate release asset, and silently undoing a hardening control the operator deliberately turned on. That is real and worth fixing, but it is not code execution. Precondition-wise it needs the operator to have configured an allowlist and the attacker to hold repository write, the same constraints as the parent, so the constraints do not push it above the parent either.

The takeaway for me was to verify the last mile of impact before anchoring severity. The serving path was one file away and it changed the whole rating.
...
When a fix lands, the first variant-hunting move is to ask which other callers reach the same write. The CVE-2025-68939 fix guarded the API rename, and the web form reached the identical `UpdateAttachmentByUUID` write with no guard at all. Two doors to one action, and the maintainer only locked one of them.

The second lesson is on severity honesty. I anchored to the parent's High on an XSS assumption I had not tested, and the serving layer defeated it. Checking the actual serve path turned a wrong High into a correct Medium, and correcting yourself in front of the triager is cheaper than defending a number you cannot back.

## Disclosure

Reported privately to the Gitea security team. Confirmed by the maintainers. Severity settled at Medium after discussion. CVE-2026-58428 assigned. Fixed in the Gitea 1.27.0 release. Public disclosure after that release shipped.

Thanks to the Gitea maintainers for the triage and for the pushback on scoring.
